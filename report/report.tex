...
32 atoms are taken for all the studies described, which is sufficient to adequately describe the system. The temperature effect on lattice is studied with observation of melting phenomena. For ease of calculation scaled parameters are used in this study.
\end{abstract}
...
\end{equation}
\section{Results}
The classical Molecular Dynamics simulation is implemented in Python and Fortran. The FCC lattice generation part is done in Python then Fortran is used for all the calculations. Again Python is used to generate all the plots. Fortran is used for velocity initialization to final energy and lattice position calculations. Code is available in github at \url{https://github.com/tridip66/molecular_dynamics/tree/master/src}. The simulation tries to keep the temperature constant, so steady kinetic energy is observed. The total energy of the system fluctuates around a steady state. It can be observed in the Figure 1 which is ran for $ 10ps$ at $T = 1$. For visualization of crystal lattice VESTA has been used.
\\
\begin{figure}[p]
  \centering
  \includegraphics[scale=0.4]{figures/Energyplot}% Imports a figure - does not automatically scale
  \caption{\label{fig:epsart} Energy vs time plot at T = 1.0 for 10 ps of simulation performed}
\end{figure}
\\
It can be observed that the fluctuations are subsided in Figure 2, when the target temperature is low $T = 0.1$ .
\\
\begin{figure}[p]
  \centering
  \includegraphics[scale=0.4]{figures/Energy0dot1}% Imports a figure - does not automatically scale
  \caption{\label{fig:epsart} Energy vs time plot at T = 0.1 for 4.34 ps of simulation}
\end{figure}
\\
The temperature fluctuation with simulation time is shown in Figure 3. The goal for temperature was set at T = 1.0. The temperature varies nicely around the goal.
\\
\begin{figure}[p]
  \centering
...
\begin{figure}[p]
  \centering
  \includegraphics[scale=0.5]{figures/MomPlot}% Imports a figure - does not automatically scale
...
\begin{figure}[p]
  \centering
  \includegraphics[scale=0.5]{figures/VelPlot}% Imports a figure - does not automatically scale
  \caption{\label{fig:epsart} Velocity fluctuation with time at T=1.0}
\end{figure}
\\
\subsection{Visualization of Melting}
The final position of the atoms in the crystal lattice are recorded after a 4.34 ps of simulation run. As the time is sufficient to reach the steady temperature for the system. In Figure 6 final atom postions are displayed for four different temperatures $ (T = 0.0, 0.1, 0.5, 1.0)$. At temperature $ T = 0.5$ it is clear that lattice vibation increases and at $T = 1.0 $ the periodicity of the system is lost. This can be assumed as melting of the structure. VESTA is used for the visualization of the structures. For better capturing of melting root mean square displacement can be plotted at different temperature. Also, from coordination number decreament with increasing temperature can find the melting point. 
\\
\begin{figure}[p]
  \centering
  \includegraphics[trim = 10mm 70mm 10mm 10mm, scale=0.5]{figures/Lattice}% Imports a figure - does not automatically scale
